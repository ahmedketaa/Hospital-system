...
const ReportModal = ({ show, handleClose, report }) => {
    ...
    const downloadPDF = () => {
        ...
        doc.text(`Doctor: ${report.doctorName} (${report.doctorSpecialization})`, headerX + padding, headerY + padding + 30);
        ...
        doc.text(`Date of Examination: ${new Date(report.dateOfExamination).toLocaleDateString()}`, 10, headerY + headerHeight + 60);
        doc.text(`Time of Examination: ${report.timeOfExamination}`, 10, headerY + headerHeight + 70);
      
        doc.save(`Prescription-${new Date(report.dateOfExamination).toLocaleDateString()}.pdf`);
      };
    ...
    <Modal show={show} onHide={handleClose} className={styles.modal}>
      ...
      <Modal.Body>
        <div className={styles.header}>
          ...
          <h6><strong>Doctor:</strong> {report.doctorName} ({report.doctorSpecialization})</h6>
        </div>
        ...
        <div className={styles.details}>
          ...
          <p><strong>Date of Examination:</strong> {new Date(report.dateOfExamination).toLocaleDateString()}</p>
          <p><strong>Time of Examination:</strong> {report.timeOfExamination}</p>
        </div>
      </Modal.Body>
      <Modal.Footer>
